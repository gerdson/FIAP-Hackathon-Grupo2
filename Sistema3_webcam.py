...
with gr.Blocks(title="Sistema de Segurança Avançado") as interface:
    ...
    with gr.Tabs():
        ...
        # Aba de Webcam
        with gr.Tab("🌐 Webcam"): # Aba Webcam adicionada aqui
            entrada_webcam = gr.Video(label="Transmissão ao Vivo", sources=['webcam'], streaming=True) # Componente Webcam
            botao_webcam = gr.Button("Iniciar Monitoramento") # Botão Webcam
            saida_webcam = gr.Image(label="Visualização da Webcam") # Saída Image para Webcam
            texto_webcam = gr.Textbox(label="Alertas da Webcam") # Saída Textbox para Webcam
    ...
    botao_webcam.click( # Conexão para o botão Webcam
